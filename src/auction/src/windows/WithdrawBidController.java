...
public class WithdrawBidController {
    ...
    void confirm(ActionEvent event) throws IOException, InterruptedException, NumberFormatException, ClassNotFoundException {
    	...
    	if (aid.getText().compareTo("") == 0) {
    		this.fail(x, y, bidWindow, "Withdraw failed", "Auction was", "not found");
			return;
		}
		Auction auction = MainRun.getAuction();
		int decision = auction.WithdrawBid(username, Integer.parseInt(aid.getText()));
		if (decision == 0) {
			bidWindow.close();
			...
			MainRun.getAuction().setListener(-6);
			...
			this.fail(x, y, bidWindow, "Withdraw failed", "No auction", "was found");
			return;
		}
		else if (decision == -2) {
			this.fail(x, y, bidWindow, "Withdraw failed", "You have no bid", "on this auction");
			return;
		}
    }
